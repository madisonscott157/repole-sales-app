    ...
    if (req.method === 'POST') {
      ...
      if (action === 'request') {
        ...
        if (lockData && Object.keys(lockData).length > 0) {
          // Lock exists, check if expired (3 minutes = 180000ms)
          const now = Date.now();
        ...
        // Admin override - force clear lock and create new one
        await kv.del(`lock:${saleId}`);
        
        // Create new admin lock
        const adminLockInfo = {
          userId: userId,
          timestamp: Date.now().toString(),
          saleId: saleId,
        ...
        await kv.hset(`lock:${saleId}`, adminLockInfo);
        
        return res.status(200).json({ 
          success: true, 
          message: 'Admin edit lock acquired - all users kicked out'
        });
